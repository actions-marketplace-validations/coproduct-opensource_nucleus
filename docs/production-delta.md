...
| Item | Status | Detail |
|------|--------|--------|
| VMM build acceptance (escape-class CVEs) | Done | Nucleus's isolation is delegated to Firecracker + jailer, so the installed VMM build is a security-relevant input. It was unchecked: `setup` pinned **1.14.1** and `doctor` compared with `String::contains` — an *equality* test that flagged a newer **patched** build exactly as loudly as an older vulnerable one, and no launch path checked anything. `nucleus_spec::vmm_version` now applies a **floor plus a known-vulnerable denylist**, and `spawn_firecracker_pod` refuses rather than warns. A floor alone is insufficient and that is tested: CVE-2026-5747 affects `>=1.13.0, <=1.14.3` **and 1.15.0**, so `>= 1.14.4` would accept the vulnerable 1.15.0. Pin moved to **1.16.1** — the first release with the fix, and the build the jailer argv was already validated against. Fails closed: an unrunnable or unparseable VMM is refused, never assumed safe. **Limit:** this reads a version the binary reports about itself; a replaced or lying binary defeats it entirely. That is attestation, and is not done. |
| Guest PCI posture (CVE-2026-5747 defence) | Done | Two independent halves, neither previously stated. **Host:** nucleus never passes `--enable-pci`, so Firecracker's vulnerable virtio-PCI transport is never activated (MMIO is the default and is unaffected) — now pinned by `jailer_argv_never_enables_the_pci_transport`. **Guest:** `pci=off` on the kernel cmdline. The guest half was **broken for any spec supplying `image.boot_args`**: `pci=off` lived only in the `default_args` literal, which that branch discards wholesale, while `ipv6.disable=1` immediately below was correctly enforced by appending. `enforce_pci_off` now applies after every cmdline-building branch and strips a spec-supplied `pci=` rather than honouring it (`from_spec` returns `Self`, so there is no error channel). |
...
| Effect receipts (witness) | Partial | Every mediation decision through `PolicyEnforced` appends a leaf to an RFC 6962 Merkle log naming the `(Operation, SinkClass)` and the outcome. Supports **inclusion** proofs (`O(log n)`), **consistency** proofs, an Ed25519-signed checkpoint, and a **witness** verification core (C2SP `tlog-witness` checks) that refuses to cosign a checkpoint which is not a provable append-only extension — closing split-view/fork detection. Append-only by construction. **Open:** the witness is a verification core, not a running service (no HTTP, no persistence, no origin multiplexing); no public anchoring; the signing key is caller-supplied and **not bound to a measured binary**; `run_argv`, `run_argv_async` and `NetEffect::fetch` spend below this layer and are not witnessed. |
